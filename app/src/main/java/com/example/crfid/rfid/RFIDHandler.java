...
import com.example.crfid.HomePage;
...
import com.example.crfid.ReadMaterial;
...
//    TextView textView;
    ...
    private RFID_Context context;
    // general
    private int MAX_POWER = 270;

    public
    void onCreate ( RFID_Context activity) {
        // application context
        context = activity;
        // Status UI
        MaterialTagPair_ItemDetail act= new MaterialTagPair_ItemDetail ();
//        textView = act.rFIDStatusText;
        // SDK
        InitSDK ( );
    }
    ...
    String Defaults () {
        ...
        return "Default settings applied with power: " + config.getTransmitPowerIndex ( );
    }
    ...
    void disconnect () {
        ...
        try {
            if ( reader != null ) {
                ...
                reader.disconnect ( );
                context.runOnUiThread2 ( new Runnable ( ) {
                    @Override
                    public
                    void run () {
//                        textView.setText ( "Disconnected" );
    ...
    class CreateInstanceTask extends AsyncTask<Void, Void, Void> {
        ...
        Void doInBackground ( Void... voids ) {
            ...
            readers = new Readers (  context.getContext2 () ,
                    ENUM_TRANSPORT.ALL );
            try {
                availableRFIDReaderList = readers.GetAvailableRFIDReaderList ( );
            }
